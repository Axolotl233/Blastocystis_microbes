dist.JSD <- function(inMatrix, pseudocount=0.0000001, ...) {
...
set.seed(12345)
out_dir <- paste("output/enterotype/","all",sep = "")
dir.create(out_dir,showWarnings = F,recursive = T)
load("workfile/z.phyloseq.Rdata")

tmp_meta <- metadatadf %>% filter(Pattern_class %in% pattern_type[c(1:3)])
...
tmp_otu_genus <- noise_removal(tmp_otu_genus,percent = 0.15,low = 0.01)
...
tmp_res <- tmp_meta %>% mutate(PC1 = tmp_pcoa_res_matrix[,1],PC2 = tmp_pcoa_res_matrix[,2]) %>% 
  dplyr::select(PC1,PC2,enterotype,Pattern_class,ID,species)
tmp_res$enterotype <- as.character(tmp_res$enterotype)
write_csv(tmp_meta,paste(out_dir,"entertype.csv",sep = '/'))

tmp_res_f <- tmp_res[tmp_res$species==sp[3],]
tmp_res_f <- tmp_res_f[tmp_res_f$Pattern_class %in% pattern_type[c(1:3)],]
tmp_meta_f <- tmp_meta[tmp_meta$species==sp[3],]
tmp_meta_f <- tmp_meta_f[tmp_meta_f$Pattern_class %in% pattern_type[c(1:3)],]
...
ggplot(data=tmp_res_f)+
  stat_ellipse(data=tmp_res_f,geom = "polygon",aes(x=PC1,y=PC2,fill=enterotype),level = 0.9,alpha=0.05)+
...
ggsave(paste(out_dir,"mfas_pcoa.enterotype.pdf",sep = '/'),height = 5,width = 6)
...
for(i in 1:length(res_et)){
  ...
  tmp_plot <- inner_join(tmp_res_f,tmp_d,by = "ID")
...
ggsave(paste(out_dir,"mfas_entertotype.sp.pdf",sep = '/'),width = 6,height = 6)
...
ggsave(paste(out_dir,"mfas_entertotype.ratio2.pdf",sep = '/'),width =6,height = 8)
...
ggplot(tmp_sankey_p, aes(x = x, next_x = next_x, node = node, next_node = next_node, fill = factor(node), label = node)) +
  ...
  labs(x = "")
ggsave(paste(out_dir,"mfas_entertotype.ratio.pdf",sep = '/'),width = 7,height = 5)
...
ggsave(paste(out_dir,"mfas_entertotype.parasite_load_pattern.pdf",sep = '/'),width = 6,height = 4)
...
ggsave(paste(out_dir,"mfas_entertotype.parasite_load.pdf",sep = '/'),width = 3,height = 4)
tmp_res_f %>% group_by(enterotype,Pattern_class) %>% summarise(n())
tmp_maaslin2_data <- as.data.frame(t(tmp_meta_f %>% dplyr::select(parasite_load)))
Maaslin2(
  input_data = tmp_maaslin2_data, 
  input_metadata = tmp_res_f, 
  min_prevalence = 0,
  normalization = "NONE",
  transform = "LOG",
  analysis_method = "LM" ,
  output = paste(out_dir,"mfas_entertotype_cov",sep = '/'), 
  fixed_effects = c("enterotype","sex","age","bmi","paraload_total"),
  reference = c("enterotype,ET1")
)

tmp_res_f <- tmp_res[tmp_res$species==sp[4],]
tmp_res_f <- tmp_res_f[tmp_res_f$Pattern_class %in% pattern_type[c(1:3)],]
tmp_meta_f <- tmp_meta[tmp_meta$species==sp[4],]
tmp_meta_f <- tmp_meta_f[tmp_meta_f$Pattern_class %in% pattern_type[c(1:3)],]
tmp_jsd_f <- tmp_jsd_data[tmp_meta_f$ID,tmp_meta_f$ID]
tmp_adonis_res <- adonis2(tmp_jsd_f~enterotype+Pattern_class+age+sex+bmi+parasite_load_log, tmp_meta_f,by = "term")
tmp_res_f$Pattern_class <- factor(tmp_res_f$Pattern_class,levels = pattern_type[c(1:3)])
...
ggsave(paste(out_dir,"mmul_pcoa.enterotype.pdf",sep = '/'),height = 5,width = 6)
...
plot_grid(plot_l[[1]],plot_l[[2]],nrow = 1)
ggsave(paste(out_dir,"mmul_entertotype.sp.pdf",sep = '/'),width = 6,height = 6)
...
ggsave(paste(out_dir,"mmul_entertotype.ratio2.pdf",sep = '/'),width =6,height = 8)
tmp_sankey_p <- tmp_meta_f %>% dplyr::select(Pattern_class,enterotype) %>% make_long(enterotype,Pattern_class)
ggplot(tmp_sankey_p, aes(x = x, next_x = next_x, node = node, next_node = next_node, fill = factor(node), label = node)) +
  geom_alluvial(flow.alpha = 0.3,width = 0.2)+
  geom_alluvial_text(size = 3, color = "white")+
  ...
  labs(x = "")
ggsave(paste(out_dir,"mmul_entertotype.ratio.pdf",sep = '/'),width = 5,height = 5)
...
ggsave(paste(out_dir,"mmul_entertotype.parasite_load_pattern.pdf",sep = '/'),width = 6,height = 4)
...
Maaslin2(
  ...
  transform = "LOG",
  analysis_method = "LM" ,
  output = paste(out_dir,"mmul_entertotype_cov",sep = '/'), 
  fixed_effects = c("enterotype","sex","age","bmi","paraload_total"),
  reference = c("enterotype,ET1")
)

ggsave(paste(out_dir,"mmul_entertotype.parasite_load.pdf",sep = '/'),width = 4,height = 3)
tmp_res_f %>% group_by(enterotype,Pattern_class) %>% summarise(n())
