...
set.seed(19970519)
out_dir <- paste("output/randomforest",sp[3],sep = "/")
dir.create(out_dir,showWarnings = F,recursive = T)

load("workfile/z.phyloseq.Rdata")
phyloseqin_used <-  subset_samples(phyloseqin, species == sp[3])
...
model_imp_pvalue <- as.data.frame(model_classify_pm$pval[,,2]) %>% select(`%IncMSE`)
...
plot_grid(plot_l1[[1]],plot_l1[[2]],plot_l1[[3]],plot_l1[[4]],
          ...
          plot_l1[[13]],plot_l1[[14]],plot_l1[[15]],
          plot_l1[[16]],plot_l1[[17]],plot_l1[[18]],plot_l1[[19]],
          plot_l1[[20]],plot_l1[[21]],plot_l1[[22]],
          nrow = 5)
...
phyloseqin_used <-  subset_samples(phyloseqin, species == sp[4])
tmp_otu <- as.data.frame(otu_table(phyloseqin_used))
tmp_meta <- metadatadf %>% dplyr::filter(class != "control") %>%
  dplyr::select(ID,parasite_load_log)
tmp_meta <- tmp_meta[tmp_meta$ID %in% colnames(tmp_otu), ] 
tmp_otu <- tmp_otu[,colnames(tmp_otu)%in%tmp_meta$ID]
identical(tmp_meta$ID,colnames(tmp_otu))
tmp_data <- as.data.frame(t(tmp_otu))
...
row.names(tmp_data) <- tmp_data$ID
tmp_data <- tmp_data %>% select(!ID) 
tmp_data_pre_f <- tmp_data[,c(model_imp_sort_top$predictors,"parasite_load_log")]
test_predict <- predict(model_train_2,tmp_data_pre_f[,1:(ncol(tmp_data_pre_f)-1)])
defaultSummary(data.frame(obs = tmp_data_pre_f$parasite_load_log,
                          pred = test_predict))
r_list_pre <- rf_estimation(tmp_data_pre_f$parasite_load_log,test_predict)
plot_grid(r_list_pre[[1]],r_list_pre[[2]],r_list_pre[[3]],nrow = 2,align = "hv")
ggsave(paste(out_dir,"rf_pre_mmul.pdf",sep = '/'),width = 8,height = 8.5)