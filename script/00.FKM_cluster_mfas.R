...
#d_1 %<>% drop_na(parasite_load) %>% filter(ID != "SX34") %>% select(ID,parasite_load,species,ST1,ST2,ST3)
...
d_cluster_res <- batch_cvalue(d_12,c(2:5),c(4:7),paste(out_dir,"cvalue_mfas",sep = "/"))
save(d_cluster_res,file = "workfile/FKM_cluster_mfas.Rdata")
...
plot(c(2,3,4), d_area_d, type = "b", pch = 16, col = "blue",
...
r1 <- read.csv("output/subtype_class/cvalue/5/tern.csv")
r1 <- r1 %>%
  mutate(
    Pattern_class1 = case_when(
  ...
  )%>% select(ID,Pattern_class1)
r2 <- read.csv("output/subtype_class/cvalue_mfas/3/tern.csv")
r2 <- r2 %>%
  mutate(
    Pattern_class2 = case_when(
      Pattern == "Pattern_1" ~ "ST1_dom",
      Pattern == "Pattern_2" ~ "ST3_dom",
      Pattern == "Pattern_3" ~ "Balance",
      Pattern == "Pattern_4" ~ "ST2_dom",
  ...
  ) %>% select(ID,Pattern_class2)
r_all <- inner_join(r1,r2,by = "ID")
r_all[r_all$Pattern_class1 != r_all$Pattern_class2,]
