...
out_dir <- paste("output/different_analysis/",sp[4],sep = "")
dir.create(out_dir,showWarnings = F,recursive = T)

phylo_used <- subset_samples(phyloseqin, species == sp[4])
phylo_used <- subset_samples(phylo_used,Pattern_class %in% pattern_type[c(1:3)])
meta_used <- metadatadf %>% filter(species == sp[4],Pattern_class %in% pattern_type[c(1:3)])
...
ggsave(paste(out_dir,"massling.pdf",sep = '/'),width = 4,height = 6)

phylo_used <- subset_samples(phyloseqin_all, species == sp[4])
phylo_used <- subset_samples(phylo_used,Pattern_class %in% pattern_type[c(1,3)])
phylo_used <- filter_taxa(phylo_used, function(x) sum(x > 0) > 0, TRUE)
d_otu <- as.data.frame(t(otu_table(phylo_used)))
